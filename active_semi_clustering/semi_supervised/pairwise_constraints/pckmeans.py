...
class PCKMeans:
    def __init__(self, n_clusters=3, max_iter=200, w=5):
    ...
    def _assign_clusters(self, X, cluster_centers, ml_graph, cl_graph, w):
        ...
        for x_i in index:
            objective_res = [self._objective_function(X, x_i, cluster_centers, c_i, labels, ml_graph, cl_graph, w) for c_i in range(self.n_clusters)]
            labels[x_i] = np.argmin(objective_res)
            distance[x_i] = np.min(objective_res)
