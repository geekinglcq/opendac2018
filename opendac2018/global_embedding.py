...
def clean_sent(s, prefix):
    ...
    return [
        '__%s__%s' % (prefix, stemer.stem(w)) for w in words
            ...
            for clust in author_papers:
                ...
                for pid in clust:
                    sam = np.random.choice(
                        clust, min(len(clust), 6), replace=False)
                    for pid_pos in sam:
                        triples.append(
                            [pid, pid_pos,
...
class GlobalModel(object):
    def __init__(self):
        ...
        self.model = Model(
            ...
            name='triple_siamese')
        self.model.compile(
            loss=triplet_loss, optimizer='adam', metrics=[accuracy])
        self.infer = Model(
    ...
    def train(self, X, retrain=True):
        if retrain:
            ...
            self.model.fit(
                ...
                callbacks=[self.early, self.checkpoint])
        else:
            self.load()
